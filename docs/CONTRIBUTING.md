...
| [SynthEngine.test.ts](../src/__tests__/SynthEngine.test.ts) | Voice diffing — common tones keep ringing, only changed notes are attacked; slot/octave transitions; waveform vs. envelope edits; the stacked oscillators — silent and untriggered while their layers are off, every live oscillator sounding the same notes and every arpeggiator step once on, the third sounding under the first with the second left off, a layer switching in and out under a held chord without restarting it, only the layer whose own waveform changes retriggering, a level or detune drag never retriggering, the first oscillator held at unity while the layers are off, what is live balanced equal-power when they are, a level turned all the way down silencing its layer and handing its share back, and all three released by `releaseAll`; the `cutoffHz` curve; the `levelFromDb` window and its `-Infinity` floor; the meter tap being read and disposed; the filter type switching without disturbing the sweep; and the effects rack — the default chain wired in order, the chorus and tremolo LFOs started, each effect holding its own amount, a reorder rewiring without leaving a node feeding the chain it was moved out of, and the timed effects' rates: seconds on the delay against Hz on the two LFOs, a locked one tracking the tempo while an unlocked one ignores it, the tempo held across a call that omits it, and the delay line allocated long enough for the longest division at the slowest tempo. Plus the arpeggiator: walking a held chord instead of sustaining it, taking over and handing back a chord that is sounding when it is switched on or off mid-hold, anchoring the clock on the chord that opens a phrase and leaving the grid alone for every chord after it — playing a change that lands inside the capture window at once and holding a later one back to the step it is nearer to — turning on through a fist until the grace runs out and anchoring again after it, leaving the clock alone for an edit, every pattern matching `arpSequence`, a rate that follows the tempo only while locked — including a tempo that arrives through `setEffects` — the gate's share of a step, a step placed at the time it was scheduled for, and nothing of itself left on the global transport after `dispose`. Plus the clock: turning with the engine rather than with whatever first needs it, retiming through either setter that carries the tempo, the metronome staying silent until it is switched on and then sounding its downbeat higher, the headroom a click or a grid takes and the lamps do not, the bar counted out to one listener and nothing published while the lamps are off, and quantization — a change held mid-bar until the mark and attacked at that mark's own time, half-bar and quarter-bar landing where the counting says, one arriving inside the capture window played at once and a later one held, a hand returning to the sounding chord cancelling the wait, only the last of several changes in a bar surviving, a fist waiting on the grid while `releaseAll` still cuts, a change let through when the grid is switched off under it, and the arpeggiator founded on the beat the chord landed on |
...
| [clock.test.ts](../src/__tests__/clock.test.ts) | The grid walked forward from every beat of the bar — a whole-bar change landing on the next one wherever it was made, a half-bar one on the next one or three, a quarter-bar one on the next beat — free play having no marks at all, every mode starting on the first beat, each grid dividing the bar rather than running past it, `beatsSinceGrid` reading forward from the −1 the engine asks about before the first beat, `beatSeconds` at three tempos, the defaults being inert, and `normalizeClock` on junk, half-built objects, a grid it does not know, a truthy string for the click and an absent key for the blink that defaults on |
| [quantizeGlyph.test.ts](../src/__tests__/quantizeGlyph.test.ts) | Every grid drawn inside its padding over a bar line that runs edge to edge, the bar marked into as many pieces as the grid divides it into, free play drawing the bar with nothing on it, the marks evenly spaced from the left edge, the downbeat's mark standing taller than the rest, and no two grids drawing the same picture |
...
| [voice.test.ts](../src/__tests__/voice.test.ts) | `layerGainsDb` silencing a layer that weighs nothing rather than leaving it merely quiet, one live layer sitting at unity whatever its level reads, an even pair and an even trio each at equal power, the summed power staying flat however the balance is set, the louder level taking the louder layer, silence when nothing weighs anything, and a level from outside the knob's range clamping; `levelsFromMix` reproducing the gains the pre-v2 crossfade gave so an old song sounds the same, scaling the louder side to a full level, and keeping float noise off what it writes; `layerDetune` folding whole octaves into the same cents as the fine detune; and the default voice shipping both stacked layers off but ready to sound like something the moment they are switched on |
...
| [settings.test.ts](../src/__tests__/settings.test.ts) | Load/save round-tripping, the section and slot arrays being pinned to length, section 1 forced on, `activeSection` falling back off a disabled section, `accidental` falling back to sharps on an unknown value, the v3 → v4 migration — chords carried over, the old key consumed once, a newer blob short-circuiting it — and the v4 → v5 migration, where the old send lands on the effects it named, `both` splits across delay and reverb, a blob with no send falls back to the old defaults rather than to silence, and a v4 blob is taken over an older v3 one; the v5 → v6 migration, where the old `mixB` crossfade is converted to the levels that reproduce its gains, the key it replaced is dropped rather than carried along, the rest of the blob survives, a blob with no mix of its own is left alone, an unreadable one falls back to the even pair, and a v4 blob is carried all the way through with its send and its mix both reshaped; and the stacked oscillators — a round trip, a blob from before the third existed picking up its five defaults, a hand-edited one normalizing, and a fractional octave offset snapping to a whole one; and the clock — a round trip, a blob from before it existed picking up defaults that hold no chord back and make no sound, a hand-edited one normalizing, a value that is not a clock at all falling back whole, and the grid travelling with a song |
...
`Loop` stub is the one piece that is not a node: it records what was scheduled and
exposes a crank the tests turn by hand, because the transport that would turn it
does not exist under the mock. There are two loops — the arpeggiator's and the
beat clock's — so the stub fills one or the other by the order the engine builds
them in, and `Volume`, the first node of a new graph, resets that count.
...
the layout needs nothing. All three oscillators draw from the same array, so a new
shape reaches the stacked one too.
...
**A new settings group in the rail:** add its id to `PANEL_GROUPS` and a label to
`PANEL_GROUP_LABELS` in [state/panel.ts](../src/state/panel.ts) — the array's
order *is* the rail's order, and `panel.test.ts` iterates it, so the round trip
and the label are covered for free. `ICONS` in
[PanelRail.tsx](../src/components/PanelRail.tsx) is an exhaustive record, so the
compiler will demand a glyph before it will build; write one in
[icons.tsx](../src/components/icons.tsx), and where the group's own controls draw
something, quote that drawing rather than inventing a second shape — `SoundIcon`,
`ArpIcon` and `TimingIcon` all call the function their picker calls. Then add a
`<section className="panel-group band-…" hidden={group !== '…'}>` to
[SettingsPanel.tsx](../src/components/SettingsPanel.tsx); the band picks the ink,
so choose it by whose parameter the group holds — a hand's, or the machine's.
