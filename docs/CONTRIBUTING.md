...
| [voice.test.ts](../src/__tests__/voice.test.ts) | `mixGainDb` silencing whichever oscillator is fully crossfaded out rather than leaving it merely quiet, the pair sitting at equal power through the middle of the travel, the summed power staying flat across the sweep, the two levels moving opposite ways, and a mix from outside the knob's range clamping; `oscBDetune` folding whole octaves into the same cents as the fine detune; and the default voice shipping the layer off but ready to sound like something the moment it is switched on |
...
the layout needs nothing. Both oscillators draw from the same array, so a new
